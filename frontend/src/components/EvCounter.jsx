...
function EvCounter({ compact = false }) {
    ...
    if (!stats) {
        return <div className="text-xs text-neutral-600">Loading the +EV counter…</div>;
    }
    ...
    const color = winning ? 'text-emerald-400' : 'text-rose-400';

    if (compact) {
        return (
            <span className="text-xs tabular-nums text-neutral-400">
                Bot <span className={`font-semibold ${color}`}>{fmtSigned(botNet)} BB</span>
                <span className="text-neutral-600">
                    {' '}({fmtRate(bbPerHand)} BB/hand) · {hands.toLocaleString()} hands
                </span>
            </span>
        );
    }
