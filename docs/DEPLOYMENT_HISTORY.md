# Deployment History & Case Study

The journey from "Flask + Vite dev setup" to the v1.0.0 production deploy
at https://allin.jianrontan.com. Preserved as the **engineering narrative
+ interview case study** — the cost model, the stack choices, the security
hardening pass, the v1-surface code map, the WebAssembly analysis, the
+EV-leaderboard design. Decisions are recorded with their original
timestamps so the *what we chose and why* stays legible.

For the **current operating runbook** (env vars, Cognito setup, deploy
steps, post-launch ops, hardening checklist) — see
[DEPLOYMENT_RUNBOOK.md](DEPLOYMENT_RUNBOOK.md).
For **day-to-day operations** (rotating IAM keys, upgrading the blueprint,
dev workflow) — `docs/private/MAINTENANCE.md` (gitignored, author-only).

Status legend (used in the original plan-doc badges below): ✅ done · 🚧 in
progress · 📅 planned. **All of D0–D4 shipped in v1.0.0**; the badges that
follow are the original plan-of-record they were authored under, left as-is
so the sequencing/reasoning narrative stays intact.

Last substantive content update: 2026-06-07.

---

## Update 2026-06-07 — what's changed since the original plan

> **⚠️ SUPERSEDED LATER THE SAME DAY (2026-06-07) — read this first.** The
> bullets below committed to building a **baked n=64 turn solver**. That
> decision was **reversed** hours later when the turn solver's **N0
> real-game gate FAILED** (it lowered exploitability but did NOT beat the
> blueprint head-to-head). **The turn bake and the NN leaf are NOT being
> built.** The shipped plan is **Rung 1 = the 25M blueprint + the river
> subgame solver only** (see the "Sequencing" section below and ROADMAP
> Phase 4 / NN_LEAF_PLAN.md). Treat the turn-bake bullets in the next
> section as historical, not the plan of record.
...
decide the bot's move and reads/writes the **DynamoDB** leaderboard.
...
5. **The container — Docker + gunicorn.** **Docker** packages app + Python + numpy + the blueprint
   ...
   bucketing (see D0). In v1.0 they're fetched from the `assets-v1` GitHub Release at image build
   time; production images embed them as a regular file COPY.
...
7. **Datastore — DynamoDB (on-demand).** A managed NoSQL DB (no server to run; pay-per-request,
   ...
   restart) and the leaderboard (`players` / `global` / `sessions` / `hands`), behind the
   `SessionStore` / `PlayerStore` / `GlobalStatsStore` / `HandStore` seams. Note the blueprint
   itself is **not** here — it's the read-only SQLite `.db` baked into the image (layer 5); it
   never changes at runtime, so it needs no database service.

(Infrastructure-as-Code via Terraform was *planned* in the original document; v1.0 instead
manages AWS resources via the console + the one-shot `scripts/provision_dynamodb.py`. A future
pass to Terraformify the Lightsail/ECR/IAM/DynamoDB resources would be additive and is in
the post-launch hardening backlog.)
...
## Deployment roadmap (original plan-of-record)

> **All of D0–D4 below shipped in v1.0.0.** The 📅 status badges that follow are
> the *original* plan-of-record they were authored under; left as-is so the
> sequencing/reasoning narrative stays intact.
...
- **Bake the postflop tables on build** (`scripts/bake_postflop_table.py`) or ship the
  ...
  by design — fine.) In v1.0 CI fetches the baked tables from a GitHub Release asset
  (`assets-v1`) rather than re-baking on every build.
- **Ship the blueprint `.db`** into the image/instance. The image's `.dockerignore` keeps only
  ...
  so no pin is required; set `ALLIN_BLUEPRINT_DB` only to override.
...
subdomain as a separate Pages deployment, $0 each.
...
favors flat-rate over pay-per-use — Lightsail is cheaper *and* simpler here. Fargate's
...
Notes:
- For the launch window, Lightsail Containers **Medium** (2 vCPU / 4 GB, ~$40/mo flat) was
  the pick; the v1.0 actual deploy is **Lightsail Containers Micro** ($10/mo) which has
  comfortably handled the launch traffic — the residency cost is the actual cost.
- The rest of the stack is ~free at this scale: **Cloudflare Pages** frontend ($0), **DynamoDB**
  on-demand (~$0 at thousands of hands).
- **All-in:** ~$10/mo Lightsail residency + ~$10/yr domain ≈ ~$130/yr total.
...
- **Recommended:** register + DNS at **Cloudflare (~$10/yr flat)**.

> **What v1.0 actually did:** `allin.jianrontan.com` → Cloudflare Pages (orange-cloud
> CNAME); `api-allin.jianrontan.com` → Lightsail (orange-cloud CNAME + Lightsail
> custom-domain ACM cert + CF Full SSL mode). The originally-planned `api.allin.jianrontan.com`
> (two-deep subdomain) was abandoned mid-launch when CF Universal SSL turned out not to
> cover two-deep subdomains; `api-allin` (flat) stays within the free cert. This is
> documented in the runbook as a "common pitfall."
...
- **DynamoDB, ECR, Docker, domain/DNS** — **unchanged**.
...
- **Per-hand state reset.** `_deal_hand` now resets `result_recorded`, `pending_translation`,
  and `recap_ts_ms` — same family of bug (per-hand anchor surviving across hands).
...
- **M3** — `BlueprintDB` now uses **thread-local** SQLite connections (the Flask thread pool no
  ...
  in** as an inspection feature — Dockerfile now bakes `ALLIN_DEBUG_OVERLAY=0` as the
  secure-by-default in the image (post-launch fix), so the live public bot's bucket can't leak
  even if the Lightsail env var is forgotten.
- **M1** — `requirements.txt` includes `gunicorn`, `waitress`, and `boto3` (pinned).

This closes the D0 "real WSGI" gap and the D2 "DynamoDBSessionStore (mandatory)" gap in code;
provisioning the table + setting the env vars in the runbook is what remained at deploy time.
...
6. Backend auth.verify_cognito_id_token(idToken):
     ...
     - checks email_verified == true (rejects unverified Google emails)
