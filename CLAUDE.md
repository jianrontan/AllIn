...
The SERVED blueprint is `blueprint_final_v2.db` (30/24/10 buckets, 194,617 info sets,
52.5M iterations, alpha=1.5, gamma=1.0) — confirm live via `/api/healthz`, which reports
the blueprint name + iteration count.
...
- The blueprint grew to ~195k info sets (194,617 in the served `blueprint_final_v2.db`; the older v1 `blueprint_final.db` has 128,177); bundling the JSON bloated the Vite build and shipped the whole strategy to every visitor.
...
# PARALLEL (data-parallel MCCFR+ across cores) — how a full blueprint is actually trained.
# CLI form (the function takes the same kwargs): resumes the same DB with --resume.
python tests/run_blueprint_trainer.py --iterations 7500000 --workers 16 --merge-every 2000 \
       --menu-mode capped --gamma 1.0 --checkpoint-every 50000 [--resume blueprint_par_*.db]
```

Training writes a timestamped `analysis/blueprints/blueprint_*.db` (parallel runs get a
`blueprint_par_*` prefix). There is **no manual promotion step** — `src/config.py:resolve_blueprint_path()`
automatically selects the active blueprint (see Key Constraints).

**Cloud / multi-day runs:** a full blueprint is ~2 days, so it's trained on a 16 vCPU / 64 GB box via
`scripts/train_on_cloud.sh` (deps → re-fit + bake postflop tables → chunked parallel training with BR/LBR
tracking → bundle the 5 serving artifacts). Flow + parallel mechanics: [docs/TRAININGFLOW.md](docs/TRAININGFLOW.md);
the EC2 provision/connect/monitor/teardown runbook is the private `docs/private/EC2_TRAINING_RUNBOOK.md`.
...
- **Decoupled preflop buckets — 169 fine (LOSSLESS) / 10 coarse** (imperfect recall, Libratus/Pluribus structure). The fine and coarse ids are two quantilings of the *same* Monte Carlo equity table (`scripts/compute_preflop_equity.py`), both **derived at import** in `card_abstractions.py` from the committed `_PREFLOP_EQUITY` (no bucket literal to maintain):
...
- **Phase 2 — Potential-aware postflop buckets** ✅ done. The distribution-aware abstraction:
  ...
  (`PostflopV2`): **30/24/10** in the served v2 blueprint (assets-v2 cutover DONE), **20/16/10**
  in the older v1. Replaced the old 8-bucket heuristic.
...
- **Phase 6 — Opponent EXPLOITATION** ⏳ built + SERVABLE behind flags (default OFF), validated locally,
  ...
  live last-N over a hard gate + batch; the 20/16→30/24 abstraction is now resolved — v2 is fit and
  SERVED in prod, so the abstraction-mismatch guard no longer blocks enabling exploit).
