...
**Summary.** UptimeRobot recorded **20 incidents totalling 8h30m36s**, all root-caused as "Connection Timeout", clustered on **2026-09-22 18:03–23:54 GMT+8** (longest 49m31s). `/api/healthz` response time over that 24h: **min 1062 ms, avg 3731 ms, max 26444 ms** from the North America probe. The investigation could not determine *where* the time went, because the container log answers neither "was the container slow?" nor "did anything error?" — the access log has **no duration field**, and gunicorn's error log goes to **stderr, which Lightsail appears not to surface**. The duration half is fixed here; the stderr question and the incident's own root cause both remain open (below).
...
**Fix (shipped).** `docker-entrypoint.sh`: add `--access-logformat` carrying `%(D)s` (request microseconds), overridable via `ALLIN_ACCESS_LOGFORMAT`. The error log **stays on stderr** — see the failed first attempt below.

**FAILED FIRST ATTEMPT — `--error-logfile /dev/stdout` broke the deploy (same day).** The first version of this fix also pointed the error log at `/dev/stdout` and added `--capture-output`, reasoning that `/dev/stdout` is `/proc/self/fd/1` and the process owns fd 1, so non-root `USER allin` could write it. That reasoning was about *permissions* and missed the actual constraint: **gunicorn 23 opens a non-`-` error log with `open(cfg.errorlog, 'a+')` (`glogging.py:205`), and `'a+'` requires a SEEKABLE file.** A container's stdout is a **pipe**, so startup dies in `Arbiter.setup()` with `io.UnsupportedOperation: File or stream is not seekable` — *before* any worker boots and before anything is logged. Lightsail deployment **v18 went `FAILED`**, the container never listened, health checks never passed, and the rolling deploy correctly kept v17 (`d1ff1b5`) serving — **prod was never down from this**. The `ALLIN_ERROR_LOGFILE=-` "escape hatch" shipped alongside it was useless by construction: a container that won't start can't read the env var meant to save it. Reproduced in 30 s with `gunicorn==23.0.0` on a plain Linux shell — it would have failed on *any* host, nothing container-specific.

**Why CI didn't catch it.** The Docker `test` stage runs pytest **against the image but never starts the entrypoint**, so a gunicorn invocation that cannot boot passes every test and fails only at deploy. Shell syntax (`sh -n`) and variable expansion were both verified; neither executes gunicorn. **The gap is that nothing in CI ever starts the server.** A ~10-line smoke step — boot the prod image, curl `/api/healthz`, assert 200 — would have caught this before the push, and would catch every future entrypoint regression.
